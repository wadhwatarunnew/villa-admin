<?php
	...
	else if(isset($_GET['Action']) && $_GET['Action'] == "GetHomePage")
	{
		...
		// $Row 	= mysqli_fetch_assoc($Result);
		// $FinalArray['TopSection']['logo'] 		= "villadashboard/".$LogoRow['path'];
		...
		$FinalArray['AboutSection']['image'] 	= $FinalArray['TopSection']['image'];
        ...
        while($PostRow = mysqli_fetch_assoc($PostResult))
        {
          	...
          	$FinalArray['LatestPosts'][$i]['content'] 	= implode(" ", array_splice($Pieces, 0, 12));
           	...
           	{
           		$FinalArray['LatestPosts'][$i]['image'] = "villadashboard/".$PostRow['local_path'];
           	}
           	else
           	{
           		$FinalArray['LatestPosts'][$i]['image'] = "villadashboard/".$PostRow['image'];
           	}
	...
	else if(isset($_GET['Action']) && $_GET['Action'] == "GetProjectsPage")
	{
		...
		$FinalArray['ProjectInfo']['title'] 	= $ProjectRow['title'];
	   	...
	   	$FinalArray['ProjectInfo']['image'] = "villadashboard/".$ProjectRow['local_path'];
	   	if(isset($ProjectRow['image']) && $ProjectRow['image'] != '')
	   	{
	   		$FinalArray['ProjectInfo']['image'] = "villadashboard/".$ProjectRow['image'];
	   	}
	...
	else if(isset($_GET['Action']) && $_GET['Action'] == "GetYoutubePage")
	{
		...
		while($VideoRow = mysqli_fetch_assoc($YoutubeResult))
		{
			...
			$FinalArray['Videos'][$i]['image'] = "villadashboard/".$VideoRow['local_path'];
		   	if(isset($VideoRow['image']) && $VideoRow['image'] != '')
		   	{
		   		$FinalArray['Videos'][$i]['image'] = "villadashboard/".$VideoRow['image'];
		   	}
		   	$i++;
		}
