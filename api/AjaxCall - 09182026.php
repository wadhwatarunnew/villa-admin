<?php
	...
	if (isset($_GET['Action']) && $_GET['Action'] == "GetURLs")
	{
	    ...
	    if (mysqli_num_rows($Result) > 0)
	    {
	        ...
	        if ($Row->name_three != '' && $Row->link_three != '')
	        {
	            ...
	            if (mysqli_num_rows($CatResult) > 0)
	            {
	                ...
	                {
	                    $ResortTypes[$CategoryRow['category']][] = $CategoryRow;
	                }
	            }


	            // Build resort category tree
	            if (mysqli_num_rows($Result) > 0)
	            {
	                $i = 0;
	                while ($ResortRow = mysqli_fetch_assoc($Result))
	                {
	                    ...
	                    if (isset($ResortTypes[$CategoryName]))
	                    {
	                        $j = 0;
	                        foreach ($ResortTypes[$CategoryName] as $CategoryRow)
	                        {
	                            $CategoryTitle = preg_replace(
	                                '/[^a-z]/',
	                                '-',
	                                strtolower($CategoryRow['title'])
	                            );

	                            $CategoryTitle = str_replace(
	                                '--',
	                                '-',
	                                strtolower($CategoryTitle)
	                            );
	        ...
	        if ($Row->name_four != '' && $Row->link_four != '')
	        {
	            ...
	            if (mysqli_num_rows($CatResult) > 0)
	            {
	                while ($CategoryRow = mysqli_fetch_assoc($CatResult))
	                {
	                    $ProjectTypes[$CategoryRow['category']][] = $CategoryRow;
	                }
	            }

	            // Build project category tree
	            if (mysqli_num_rows($Result) > 0)
	            {
	                $i = 0;
	                while ($ProjectRow = mysqli_fetch_assoc($Result))
	                {
	                    $CleanURL = preg_replace(
	                        '/[^a-z]/',
	                        '-',
	                        strtolower($ProjectRow['title'])
	                    );

	                    $FinalArray['Projects']['children'][$i]['id']   = $ProjectRow['id'];
	                    $FinalArray['Projects']['children'][$i]['name'] = $ProjectRow['title'];
	                    ...
	                    if (isset($ProjectTypes[$CategoryName]))
	                    {
	                        $j = 0;
	                        foreach ($ProjectTypes[$CategoryName] as $CategoryRow)
	                        {
	                            $CategoryTitle = preg_replace(
	                                '/[^a-z]/',
	                                '-',
	        ...
	        if ($Row->name_five != '' && $Row->link_five != '')
	        {
	            ...
	            $Result = mysqli_query($con, "SELECT id, title FROM blog_inner_content");
	            if (mysqli_num_rows($Result) > 0)
	            {
	                $i = 0;
	                while ($BlogRow = mysqli_fetch_assoc($Result))
	                {
	                    $BlogURL = preg_replace(
	                        '/[^a-z]/',
	                        '-',
	                        strtolower($BlogRow['title'])
	                    );
	    ...
	    $i = 0;
	    $Result = mysqli_query($con, "SELECT name, icon, link FROM footer_follow_us ORDER BY id ASC");
	    if (mysqli_num_rows($Result) > 0)
	    {
		...
		// CONTACT INFORMATION
	    $Result = mysqli_query($con, "SELECT title, address, email, mobile FROM footer_get_in_touch");
	    if (mysqli_num_rows($Result) > 0)
	    {
		...
		// FOOTER ABOUT US
	    $Result = mysqli_query($con, "SELECT title, content FROM footer_about_us");
	    if (mysqli_num_rows($Result) > 0)
	    {
	        $Row = mysqli_fetch_assoc($Result);
	    ...
	    $Result = mysqli_query($con, "SELECT name, link FROM add_nav WHERE title='TERMS AND CONDITIONS'");
	    $Row = mysqli_fetch_assoc($Result);
	...
	if(isset($_GET['Action']) && $_GET['Action'] == "GetURLs2")
	{
		$FinalArray = array(); 
		$Result = mysqli_query($con, "SELECT * FROM header_nav");
		if(mysqli_num_rows($Result) > 0)
